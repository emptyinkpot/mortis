	...
	case method == "item/started" && itemType == "mcpToolCall":
		if c.onMessage != nil {
			c.onMessage(Message{
				...
				CallID: itemID,
				Input:  codexMCPArguments(item["arguments"]),
			})
		}

	case method == "item/completed" && itemType == "mcpToolCall":
		output := codexPayloadString(item["result"])
		if output == "" {
			output = codexPayloadString(item["error"])
		}
		if c.onMessage != nil {
			c.onMessage(Message{
				Type:   MessageToolResult,
...
func codexMCPArguments(value any) map[string]any {
	if value == nil {
		return nil
	}
...
func codexPayloadString(value any) string {
	switch v := value.(type) {
	case nil:
		return ""
	case string:
