...
func TestCodexRawItemMCPToolCall(t *testing.T) {
	...
	c.handleLine(`{"jsonrpc":"2.0","method":"item/started","params":{"item":{"type":"mcpToolCall","id":"mcp-1","server":"browser","tool":"run","arguments":{"url":"https://example.com","action":"screenshot"}}}}`)
	c.handleLine(`{"jsonrpc":"2.0","method":"item/completed","params":{"item":{"type":"mcpToolCall","id":"mcp-1","server":"browser","tool":"run","result":{"status":"ok","title":"Example Domain"}}}}`)

	if len(messages) != 2 {
		t.Fatalf("expected 2 messages, got %d", len(messages))
	}
	if messages[0].Type != MessageToolUse || messages[0].Tool != "browser.run" {
		t.Fatalf("unexpected start message: %+v", messages[0])
	}
	if messages[0].Input["url"] != "https://example.com" {
		t.Fatalf("expected browser.run input to include url, got %+v", messages[0].Input)
	}
	if messages[1].Type != MessageToolResult || messages[1].Tool != "browser.run" {
		t.Fatalf("unexpected complete message: %+v", messages[1])
	}
	if !strings.Contains(messages[1].Output, `"title":"Example Domain"`) {
		t.Fatalf("expected MCP result payload in output, got %q", messages[1].Output)
	}
}
