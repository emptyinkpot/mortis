...
func TestRunIssueCommentAdd_WithBrowserTaskContext(t *testing.T) {
	var gotBody map[string]any
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	if err := cmd.Flags().Set("content", "please run a browser check"); err != nil {
		t.Fatal(err)
	}
	if err := cmd.Flags().Set("url", "https://example.com/comment"); err != nil {
		t.Fatal(err)
	}
	if err := cmd.Flags().Set("allow-host", "example.com"); err != nil {
		t.Fatal(err)
	}
	if err := cmd.Flags().Set("purpose", "comment-browser-task"); err != nil {
		t.Fatal(err)
	}

	if err := runIssueCommentAdd(cmd, []string{"issue-1"}); err != nil {
		t.Fatalf("runIssueCommentAdd: %v", err)
	}

	if gotBody["content"] != "please run a browser check" {
		t.Fatalf("expected content in body, got %#v", gotBody["content"])
	}
	...
	if browserRun["url"] != "https://example.com/comment" {
...
func TestRunIssueCreateIncludesBrowserTaskContext(t *testing.T) {
	var gotBody map[string]any
	...
	if err := cmd.Flags().Set("title", "browser issue create"); err != nil {
		t.Fatal(err)
	}
	if err := cmd.Flags().Set("url", "https://example.com/create"); err != nil {
		t.Fatal(err)
	}
	if err := cmd.Flags().Set("allow-host", "example.com"); err != nil {
		t.Fatal(err)
	}
	if err := cmd.Flags().Set("purpose", "issue-create-browser-task"); err != nil {
		t.Fatal(err)
	}
	...
	if browserRun["url"] != "https://example.com/create" {
		t.Fatalf("expected browser url preserved, got %#v", browserRun["url"])
	}
}
