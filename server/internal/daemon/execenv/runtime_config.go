...
func buildMetaSkillContent(provider string, ctx TaskContextForEnv) string {
	...
	b.WriteString("- `multica browser run --url <url> --title-contains <text> --selector <css>` — Run one local browser-manager task against the system Edge/Chrome and capture screenshots as JSON output\n")
	...
	b.WriteString("## Multi-Agent Discussion Mode\n\n")
	b.WriteString("If the triggering comment contains `DISCUSSION_MODE`, treat it as a structured multi-agent planning thread rather than a normal one-off request.\n\n")
	b.WriteString("- Follow only the role assigned to you in that comment.\n")
	b.WriteString("- Keep replies concise and decision-focused.\n")
	b.WriteString("- When the protocol says to hand off, explicitly @mention the next agent so Multica re-triggers them.\n")
	b.WriteString("- If you are not the active hand-off target, wait instead of jumping in again.\n")
	b.WriteString("- Do not start editing code during discussion mode unless a later comment explicitly switches to execution.\n")
	b.WriteString("- If you are the moderator, end the thread with a `FINAL IMPLEMENTATION PLAN` comment that names the chosen approach, files, ordered steps, risks, and tests.\n\n")
