...
func TestExecuteAndDrainCollectsBrowserRunTool(t *testing.T) {
	t.Parallel()

	d := newTestDaemon(t)
	fb := &fakeBackend{
		messages: [][]agent.Message{{
			{Type: agent.MessageToolUse, Tool: browserRunToolName, CallID: "tool-1"},
			{Type: agent.MessageToolResult, Tool: browserRunToolName, CallID: "tool-1", Output: "ok"},
		}},
		results: []agent.Result{{
			Status: "completed",
			Output: "verified",
	...
	result, summary, err := d.executeAndDrain(context.Background(), fb, "prompt", agent.ExecOptions{}, slog.Default(), "task-browser")
	if err != nil {
	...
	if summary.Count != 1 {
		t.Fatalf("expected 1 tool use, got %d", summary.Count)
	}
	if !summary.HasTool(browserRunToolName) {
		t.Fatalf("expected tool summary to include %s", browserRunToolName)
	}
}
...
func TestBuildExecutionGatewayBrowserScopedTask(t *testing.T) {
	t.Parallel()

	gateway := buildExecutionGateway(Task{
		ChatSessionID: "chat-1",
		ChatMessage:   "请用浏览器打开页面，检查控制台错误并截图",
	}, "codex")

	if !gateway.requiresBrowserRun {
		t.Fatal("expected browser-scoped task to require browser.run")
	}
	if !strings.Contains(gateway.systemPrompt, browserRunToolName) {
		t.Fatalf("expected system prompt to require %s, got %q", browserRunToolName, gateway.systemPrompt)
	}
}

func TestExecutionGatewayBlocksReasoningOnlyCompletion(t *testing.T) {
	t.Parallel()

	gateway := executionGateway{
		requiresBrowserRun: true,
		systemPrompt:       "must use browser.run",
	}

	err := gateway.Enforce(agent.Result{Status: "completed", Output: "I think the page is fine"}, executionToolSummary{})
	if err == nil {
		t.Fatal("expected browser-scoped completion without browser.run to be blocked")
	}
	if !strings.Contains(err.Error(), browserRunToolName) {
		t.Fatalf("expected error to mention %s, got %v", browserRunToolName, err)
	}
}

func TestExecutionGatewayAllowsBrowserRunCompletion(t *testing.T) {
	t.Parallel()
	...
	err := gateway.Enforce(agent.Result{Status: "completed", Output: "verified"}, executionToolSummary{
		Count: 1,
	...
	if err != nil {
		t.Fatalf("expected browser.run-backed completion to pass, got %v", err)
	}
}
