...
import (
	...
	"github.com/multica-ai/multica/server/internal/executiongateway"
...
func (d *Daemon) handlePing(ctx context.Context, rt Runtime, pingID string) {
	strategy := "full-agent"
	mode := "full-agent"
	if rt.Provider == "openclaw" {
		strategy = "openclaw-health-then-full-agent"
	}
	d.logger.Info("ping requested", "runtime_id", rt.ID, "ping_id", pingID, "provider", rt.Provider, "strategy", strategy)
	...
	if rt.Provider == "openclaw" {
		d.logger.Info("ping health probe starting", "runtime_id", rt.ID, "ping_id", pingID, "mode", "openclaw-health", "strategy", strategy)
		healthCtx, cancel := context.WithTimeout(ctx, 15*time.Second)
		output, err := d.runOpenClawHealthPing(healthCtx, entry.Path)
		cancel()
		durationMs := time.Since(start).Milliseconds()
		if err == nil {
			d.logger.Info("ping completed", "runtime_id", rt.ID, "ping_id", pingID, "mode", "openclaw-health", "strategy", strategy, "duration_ms", durationMs)
			d.client.ReportPingResult(ctx, rt.ID, pingID, map[string]any{
				"status":      "completed",
				"output":      output,
		...
		d.logger.Warn("ping health probe failed; falling back to full agent ping", "runtime_id", rt.ID, "ping_id", pingID, "attempt_mode", "openclaw-health", "next_mode", mode, "strategy", strategy, "health_duration_ms", durationMs, "error", err)
	}

	d.logger.Info("ping full agent probe starting", "runtime_id", rt.ID, "ping_id", pingID, "mode", mode, "strategy", strategy)
	...
	if err != nil {
		d.logger.Warn("ping failed before full agent execution", "runtime_id", rt.ID, "ping_id", pingID, "mode", mode, "strategy", strategy, "error", err)
		d.client.ReportPingResult(ctx, rt.ID, pingID, map[string]any{
			"status":      "failed",
			"error":       err.Error(),
			"duration_ms": time.Since(start).Milliseconds(),
	...
	if err != nil {
		d.logger.Warn("ping failed launching full agent probe", "runtime_id", rt.ID, "ping_id", pingID, "mode", mode, "strategy", strategy, "error", err)
		d.client.ReportPingResult(ctx, rt.ID, pingID, map[string]any{
			"status":      "failed",
			"error":       err.Error(),
			"duration_ms": time.Since(start).Milliseconds(),
	...
	case <-pingCtx.Done():
		d.logger.Warn("ping timed out waiting for result", "runtime_id", rt.ID, "ping_id", pingID, "mode", mode, "strategy", strategy)
		d.client.ReportPingResult(ctx, rt.ID, pingID, map[string]any{
			"status":      "failed",
	...
	if result.Status == "completed" {
		d.logger.Info("ping completed", "runtime_id", rt.ID, "ping_id", pingID, "mode", mode, "strategy", strategy, "duration_ms", durationMs)
		d.client.ReportPingResult(ctx, rt.ID, pingID, map[string]any{
			"status":      "completed",
		...
		d.logger.Warn("ping failed", "runtime_id", rt.ID, "ping_id", pingID, "mode", mode, "strategy", strategy, "error", errMsg)
		d.client.ReportPingResult(ctx, rt.ID, pingID, map[string]any{
			"status":      "failed",
			"error":       errMsg,
			"duration_ms": durationMs,
...
func (d *Daemon) runOpenClawHealthPing(ctx context.Context, execPath string) (string, error) {
	if execPath == "" {
		execPath = "openclaw"
	}

	cmd := exec.CommandContext(ctx, execPath, "health", "--json", "--timeout", "10000")
	output, err := cmd.CombinedOutput()
	if err != nil {
		trimmed := strings.TrimSpace(string(output))
		if trimmed == "" {
			return "", fmt.Errorf("openclaw health failed: %w", err)
		}
		return "", fmt.Errorf("openclaw health failed: %w: %s", err, trimmed)
	}
	...
	if err := json.Unmarshal(output, &resp); err != nil {
		return "", fmt.Errorf("parse openclaw health output: %w", err)
	}
	if !resp.OK {
		if resp.Error != "" {
			return "", errors.New(resp.Error)
		}
		return "", errors.New("openclaw health returned ok=false")
	}
...
func (d *Daemon) handleTask(ctx context.Context, task Task) {
	...
	if execReq, ok, parseErr := executiongateway.ParseTaskContext(task.Context); parseErr != nil {
		err = fmt.Errorf("parse execution context: %w", parseErr)
	} else if ok {
		result, err = d.runGatewayTask(runCtx, task, *execReq, taskLog)
	} else {
...
func (d *Daemon) runGatewayTask(
	ctx context.Context,
	task Task,
	req executiongateway.ExecutionRequest,
	taskLog *slog.Logger,
) (TaskResult, error) {
	selfBin, err := os.Executable()
	if err != nil {
		return TaskResult{}, fmt.Errorf("resolve multica path: %w", err)
	}

	gw := executiongateway.New(
		taskLog,
		executiongateway.NewBrowserCLIAdapter(taskLog, selfBin),
	)

	toolName := "execution-gateway/" + string(req.Action)

	_ = d.client.ReportTaskMessages(ctx, task.ID, []TaskMessageData{
		{
			Seq:  1,
			Type: "tool_use",
			Tool: toolName,
	...
	outputJSON, _ := json.MarshalIndent(res, "", "  ")
	_ = d.client.ReportTaskMessages(ctx, task.ID, []TaskMessageData{
		{
			Seq:    2,
			Type:   "tool_result",
	...
	commentJSON, _ := json.Marshal(map[string]any{
		"action":             req.Action,
		"purpose":            req.Purpose,
		"gateway_status":     res.Status,
		"verification_level": res.VerificationLevel,
	...
	case "failed":
		if res.Message == "" {
			res.Message = "gateway action failed"
		}
		return TaskResult{}, fmt.Errorf("%s", res.Message)
	case "verified":
		return TaskResult{
			Status:  "completed",
			Comment: string(commentJSON),
	...
	default:
		return TaskResult{}, fmt.Errorf("unknown gateway result status: %s", res.Status)
	}
}
...
func (d *Daemon) runTask(ctx context.Context, task Task, provider string, taskLog *slog.Logger) (TaskResult, error) {
	...
	gateway := buildExecutionGateway(task, provider)
	...
	if browserManagerScript := resolveBrowserManagerScriptPath(); browserManagerScript != "" {
		agentEnv["MULTICA_BROWSER_MANAGER_SCRIPT"] = browserManagerScript
	}
	...
	execOpts := agent.ExecOptions{
		Cwd:             env.WorkDir,
		Model:           entry.Model,
		SystemPrompt:    gateway.systemPrompt,
	...
	result, toolSummary, err := d.executeAndDrain(ctx, backend, prompt, execOpts, taskLog, task.ID)
	if err != nil {
		return TaskResult{}, err
	}
	...
	if result.Status == "failed" && task.PriorSessionID != "" && result.SessionID == "" {
		...
		retryResult, retryToolSummary, retryErr := d.executeAndDrain(ctx, backend, prompt, execOpts, taskLog, task.ID)
		if retryErr != nil {
			taskLog.Error("fresh session also failed to start", "error", retryErr)
		} else {
			result = retryResult
			result.Usage = mergeUsage(firstUsage, result.Usage)
			toolSummary = retryToolSummary
		}
	}
	...
	if err := gateway.Enforce(result, toolSummary); err != nil {
		return TaskResult{
			Status:    "blocked",
			Comment:   err.Error(),
...
func resolveBrowserManagerScriptPath() string {
	if path := strings.TrimSpace(os.Getenv("MULTICA_BROWSER_MANAGER_SCRIPT")); path != "" {
	...
	if selfBin, err := os.Executable(); err == nil {
		searchRoots = append(searchRoots, filepath.Dir(selfBin))
	}

	for _, root := range searchRoots {
		if candidate := searchBrowserManagerScriptUp(root); candidate != "" {
			return candidate
		}
	}
...
func (d *Daemon) executeAndDrain(ctx context.Context, backend agent.Backend, prompt string, opts agent.ExecOptions, taskLog *slog.Logger, taskID string) (agent.Result, executionToolSummary, error) {
	session, err := backend.Execute(ctx, prompt, opts)
	if err != nil {
		return agent.Result{}, executionToolSummary{}, err
	}
	...
	toolSummary := executionToolSummary{UsedTools: make(map[string]struct{})}
		...
		for {
			...
			case msg, ok := <-session.Messages:
				...
				case agent.MessageToolUse:
					...
					if msg.Tool != "" {
						toolSummary.UsedTools[msg.Tool] = struct{}{}
					}
					if msg.CallID != "" {
						mu.Lock()
	...
	case result := <-session.Result:
		<-drainComplete
		toolSummary.Count = toolCount.Load()
		return result, toolSummary, nil
	case <-drainCtx.Done():
		<-drainComplete
		toolSummary.Count = toolCount.Load()
		return agent.Result{
