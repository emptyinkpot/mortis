...
func TestGetMeIncludesCurrentWorkspaceForAutoLoginUser(t *testing.T) {
	t.Setenv("MULTICA_AUTO_LOGIN_EMAIL", handlerTestEmail)
	t.Setenv("MULTICA_AUTO_LOGIN_WORKSPACE_SLUG", handlerTestWorkspaceSlug)

	w := httptest.NewRecorder()
	req := newRequest("GET", "/api/me", nil)
	testHandler.GetMe(w, req)
	if w.Code != http.StatusOK {
		t.Fatalf("GetMe: expected 200, got %d: %s", w.Code, w.Body.String())
	}

	var resp UserResponse
	if err := json.NewDecoder(w.Body).Decode(&resp); err != nil {
		t.Fatalf("GetMe: decode response: %v", err)
	}
	if resp.CurrentWorkspace == nil {
		t.Fatal("GetMe: expected current_workspace in auto-login mode")
	}
	if resp.CurrentWorkspace.ID != testWorkspaceID {
		t.Fatalf("GetMe: expected current_workspace.id %q, got %q", testWorkspaceID, resp.CurrentWorkspace.ID)
	}
	if resp.CurrentWorkspace.Slug != handlerTestWorkspaceSlug {
		t.Fatalf("GetMe: expected current_workspace.slug %q, got %q", handlerTestWorkspaceSlug, resp.CurrentWorkspace.Slug)
	}
}
...
func TestCreateIssuePersistsTaskContext(t *testing.T) {
	ctx := context.Background()
	...
	err = testPool.QueryRow(ctx,
		`SELECT context FROM agent_task_queue WHERE issue_id = $1 ORDER BY created_at DESC LIMIT 1`,
		created.ID,
	).Scan(&storedContext)
	if err != nil {
		t.Fatalf("failed to load stored task context: %v", err)
	}
