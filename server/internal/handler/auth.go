...
type UserResponse struct {
	...
	CurrentWorkspace *WorkspaceResponse `json:"current_workspace,omitempty"`
...
func userToResponseWithCurrentWorkspace(
	u db.User,
	currentWorkspace *db.Workspace,
) UserResponse {
	resp := userToResponse(u)
	if currentWorkspace != nil {
		workspaceResp := workspaceToResponse(*currentWorkspace)
		resp.CurrentWorkspace = &workspaceResp
	}
	return resp
}

func autoLoginWorkspaceForUser(
	ctx context.Context,
	queries *db.Queries,
	user db.User,
) *db.Workspace {
	...
	autoLoginEmail := strings.ToLower(strings.TrimSpace(os.Getenv("MULTICA_AUTO_LOGIN_EMAIL")))
	if autoLoginEmail == "" {
		return nil
	}

	workspaceSlug := strings.ToLower(strings.TrimSpace(os.Getenv("MULTICA_AUTO_LOGIN_WORKSPACE_SLUG")))
	if workspaceSlug == "" {
...
func (h *Handler) GetMe(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(
		w,
		http.StatusOK,
		userToResponseWithCurrentWorkspace(
			user,
			autoLoginWorkspaceForUser(r.Context(), h.Queries, user),
		),
	)
}
