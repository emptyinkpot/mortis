...
// EnqueueTaskForIssueWithContext creates a queued task for an agent-assigned
// issue and persists an optional execution-gateway payload.
func (s *TaskService) EnqueueTaskForIssueWithContext(ctx context.Context, issue db.Issue, taskContext json.RawMessage, triggerCommentID ...pgtype.UUID) (db.AgentTaskQueue, error) {
	...
	task, err := s.createIssueTask(ctx, issue.AssigneeID, agent.RuntimeID, issue.ID, issue.Priority, commentID, taskContext)
	if err != nil {
		slog.Error("task enqueue failed", "issue_id", util.UUIDToString(issue.ID), "error", err)
...
func (s *TaskService) EnqueueTaskForMention(ctx context.Context, issue db.Issue, agentID pgtype.UUID, triggerCommentID pgtype.UUID) (db.AgentTaskQueue, error) {
	return s.EnqueueTaskForMentionWithContext(ctx, issue, agentID, nil, triggerCommentID)
}

// EnqueueTaskForMentionWithContext creates a queued task for a mentioned agent
// and persists an optional execution-gateway payload.
func (s *TaskService) EnqueueTaskForMentionWithContext(ctx context.Context, issue db.Issue, agentID pgtype.UUID, taskContext json.RawMessage, triggerCommentID pgtype.UUID) (db.AgentTaskQueue, error) {
	...
	task, err := s.createIssueTask(ctx, agentID, agent.RuntimeID, issue.ID, issue.Priority, triggerCommentID, taskContext)
	if err != nil {
...
func (s *TaskService) createIssueTask(ctx context.Context, agentID, runtimeID, issueID pgtype.UUID, priority string, triggerCommentID pgtype.UUID, taskContext json.RawMessage) (db.AgentTaskQueue, error) {
	...
	if !json.Valid(taskContext) {
		return db.AgentTaskQueue{}, fmt.Errorf("task context must be valid JSON")
	}

	return s.Queries.CreateAgentTaskWithContext(ctx, db.CreateAgentTaskWithContextParams{
		AgentID:          agentID,
		RuntimeID:        runtimeID,
		IssueID:          issueID,
		Priority:         priorityToInt(priority),
		TriggerCommentID: triggerCommentID,
		Context:          append([]byte(nil), taskContext...),
	})
}
