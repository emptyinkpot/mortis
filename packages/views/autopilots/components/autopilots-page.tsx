...
5. Post the digest as a comment on this issue and @mention all workspace members`,
  ...
  {
    title: "PR 评审提醒",
    summary: "标记需要评审的滞留 Pull Request",
...
const STATUS_CONFIG: Record<string, { label: string; color: string; icon: typeof Zap }> = {
  active: { label: "启用中", color: "text-emerald-500", icon: Play },
...
function CreateAutopilotDialog({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success("自动流程已创建");
    } catch {
      toast.error("创建自动流程失败");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogTitle>新建自动流程</DialogTitle>
    ...
    <div className="flex h-full flex-col">
      ...
      <PageHeader className="justify-between px-5">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-sm font-medium">自动流程</h1>
      ...
      <div className="flex-1 overflow-y-auto">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center py-16 px-5">
            ...
            <p className="text-sm text-muted-foreground">还没有自动流程</p>
            <p className="text-xs text-muted-foreground mt-1 mb-6">
              为你的 AI 智能体安排周期任务。可直接选模板，也可从零开始。
            </p>
