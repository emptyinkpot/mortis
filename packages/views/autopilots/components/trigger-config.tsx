...
export function describeTrigger(cfg: TriggerConfig): string {
  ...
  switch (cfg.frequency) {
    case "hourly": {
      ...
      return `每小时的 ${min.toString().padStart(2, "0")} 分运行`;
    }
    case "daily":
      return `每天 ${formatTime12h(cfg.time)} ${offset} 运行`;
    case "weekdays":
      return `工作日 ${formatTime12h(cfg.time)} ${offset} 运行`;
    case "weekly":
      return `每周${DAYS_OF_WEEK[cfg.dayOfWeek]} ${formatTime12h(cfg.time)} ${offset} 运行`;
    case "custom":
