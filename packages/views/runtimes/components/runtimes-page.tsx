...
export default function RuntimesPage({ topSlot }: RuntimesPageProps) {
  ...
  const [selectedId, setSelectedIdState] = useState(() => urlRuntime);
  ...
  useEffect(() => {
    setSelectedIdState(urlRuntime);
  }, [urlRuntime]);

  const setSelectedId = useCallback((id: string) => {
    const nextId = id.trim();
    setSelectedIdState(nextId);
    const base = wsPaths.runtimes();
    const nextUrl = nextId
      ? `${base}?runtime=${encodeURIComponent(nextId)}`
      : base;
    replace(nextUrl);
  }, [replace, wsPaths]);
  ...
  useEffect(() => {
    ...
    const hasSelection = selectedId && runtimes.some((runtime) => runtime.id === selectedId);
    if (!hasSelection) {
      setSelectedId(runtimes[0]!.id);
    }
  }, [fetching, runtimes, selectedId, setSelectedId]);
        ...
        <ResizablePanel id="detail" minSize="50%">
          {selected ? (
            ...
            <div className="flex h-full flex-col items-center justify-center text-muted-foreground">
              ...
              <p className="mt-3 text-sm">请选择左侧运行时查看详情</p>
            </div>
          )}
        </ResizablePanel>
      </ResizablePanelGroup>
