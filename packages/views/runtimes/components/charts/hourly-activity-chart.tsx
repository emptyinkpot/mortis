...
const hourlyChartConfig = {
  count: { label: "任务", color: "hsl(var(--chart-2))" },
...
export function HourlyActivityChart({ runtimeId }: { runtimeId: string }) {
  ...
  return (
    <div className="rounded-lg border p-4">
      <h4 className="text-xs font-medium text-muted-foreground mb-3">每小时分布</h4>
      {loading ? (
        ...
        <div className="flex h-[140px] flex-col items-center justify-center">
          ...
          <p className="mt-2 text-xs text-muted-foreground">暂无任务数据</p>
        </div>
      ) : (
