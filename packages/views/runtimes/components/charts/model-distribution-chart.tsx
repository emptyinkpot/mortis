...
export function ModelDistributionChart({ data }: { data: ModelDistribution[] }) {
  ...
  const chartConfig = Object.fromEntries(
    data.map((d, i) => [
      d.model,
      { label: formatModelLabel(d.model), color: MODEL_COLORS[i % MODEL_COLORS.length] },
    ]),
  ) satisfies ChartConfig;

  return (
    <div className="rounded-lg border p-4">
      <h4 className="text-xs font-medium text-muted-foreground mb-3">按模型统计的令牌用量</h4>
        ...
        {data.map((d, i) => (
          <div key={d.model} className="flex items-center justify-between text-xs">
            <div className="flex items-center gap-2 min-w-0">
              ...
              />
              <span className="truncate font-mono">{formatModelLabel(d.model)}</span>
            </div>
