...
import type { Agent, RuntimeDevice, MemberWithUser, Issue } from "@multica/core/types";
import {
  Dialog,
  DialogContent,
  DialogHeader,
    ...
    <div className="flex h-full flex-col">
      ...
      {isArchived && (
        <div className="flex items-center gap-2 bg-muted/50 px-4 py-2 text-xs text-muted-foreground border-b">
          ...
          <span className="flex-1">此智能体已归档，不能再被分配或提及。</span>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        {activeTab === "env" && (
          <EnvTab
            agent={agent}
            runtimeProvider={runtimeDevice?.provider ?? null}
          ...
          <DialogContent className="max-w-sm" showCloseButton={false}>
            <div className="flex items-center gap-3">
              ...
              <DialogHeader className="flex-1 gap-1">
                <DialogTitle className="text-sm font-semibold">归档智能体？</DialogTitle>
                <DialogDescription className="text-xs">
                  &quot;{agent.name}&quot; 将被归档。归档后不能再被分配或提及，但历史记录会保留，之后仍可恢复。
                </DialogDescription>
              </DialogHeader>
            </div>
            <DialogFooter>
