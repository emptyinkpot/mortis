...
export function AgentsPage() {
  ...
  const napcatAccountsAgentId = agents[0]?.id ?? "";
  const { data: napcatAccounts = [] } = useQuery({
    queryKey: ["agents", wsId, "napcat-accounts", napcatAccountsAgentId],
    queryFn: () => api.listAgentNapcatAccounts(napcatAccountsAgentId),
    enabled: !!napcatAccountsAgentId,
    staleTime: 60_000,
  });
              ...
              <div className="flex items-center gap-1">
                {archivedCount > 0 && (
                  <Button
                    ...
                    title={showArchived ? "查看启用中的智能体" : "查看已归档智能体"}
                  >
            ...
            {filteredAgents.length === 0 ? (
              <div className="flex flex-col items-center justify-center px-4 py-12">
                ...
                <p className="mt-3 text-sm text-muted-foreground">
                  {showArchived ? "暂无已归档智能体" : archivedCount > 0 ? "暂无启用中的智能体" : "还没有智能体"}
                </p>
                {!showArchived && (
                  <Button
                    onClick={() => setShowCreate(true)}
        ...
        <ResizablePanel id="detail" minSize="50%">
          {selected ? (
            ...
            <div className="flex h-full flex-col items-center justify-center text-muted-foreground">
              ...
              <p className="mt-3 text-sm">选择一个智能体查看详情</p>
              <Button
                onClick={() => setShowCreate(true)}
                size="xs"
                className="mt-3"
