    ...
    <div className="space-y-4">
      ...
      {tasks.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-lg border border-dashed py-12">
          ...
          <p className="mt-3 text-sm text-muted-foreground">当前没有排队任务</p>
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-sm truncate ${isActive ? "font-medium" : ""}`}>
                      {issue?.title ?? `事项 ${task.issue_id.slice(0, 8)}...`}
                    </span>
                  </div>
                  <div className="mt-0.5 text-xs text-muted-foreground">
                    {isRunning && task.started_at
                      ? `已开始 ${new Date(task.started_at).toLocaleString()}`
                      : task.status === "dispatched" && task.dispatched_at
                        ? `已派发 ${new Date(task.dispatched_at).toLocaleString()}`
                        : task.status === "completed" && task.completed_at
                          ? `已完成 ${new Date(task.completed_at).toLocaleString()}`
                          : task.status === "failed" && task.completed_at
                            ? `失败 ${new Date(task.completed_at).toLocaleString()}`
                            : `排队中 ${new Date(task.created_at).toLocaleString()}`}
                  </div>
                </div>
