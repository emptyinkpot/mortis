...
export class ApiClient {
  ...
  async listAgentNapcatAccounts(id: string): Promise<NapcatAccount[]> {
    return this.fetch(`/api/agents/${id}/napcat-accounts`);
  }
  ...
  async getControlPlaneSummary(): Promise<ControlPlaneSummary> {
    return this.fetch("/api/control-plane/summary");
  }

  async getAgentOSSummary(params?: { workspace_id?: string; workspace_slug?: string }): Promise<AgentOSSummary> {
    const search = new URLSearchParams();
    if (params?.workspace_id) search.set("workspace_id", params.workspace_id);
    if (params?.workspace_slug) search.set("workspace_slug", params.workspace_slug);
    const query = search.toString();
    return this.fetch(`/api/agent-os/summary${query ? `?${query}` : ""}`);
  }
  ...
  async createRole(data: RoleRequest): Promise<RoleDefinition> {
    return this.fetch("/api/roles", {
      method: "POST",
      body: JSON.stringify(data),
    });
  ...
  async updateRole(id: string, data: RoleRequest): Promise<RoleDefinition> {
    return this.fetch(`/api/roles/${id}`, {
      method: "PUT",
      body: JSON.stringify(data),
    });
  ...
  async routeRoleMessage(data: RoleRouteMessageRequest): Promise<RoleRouteMessageResponse> {
    return this.fetch("/api/roles/route-message", {
      method: "POST",
      body: JSON.stringify(data),
    });
  }

  async listRoleInvocations(): Promise<RoleInvocationListResponse> {
    return this.fetch("/api/roles/invocations");
  }
