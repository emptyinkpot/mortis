---
title: Self-Hosting - Advanced Configuration
status: compatibility-supporting
audience: self-host operators
scope: generic advanced self-host configuration
sourceDoc: SELF_HOSTING.md
---

# Self-Hosting - Advanced Configuration

This document covers advanced configuration for self-hosted Multica deployments.
For the quick start guide, see [SELF_HOSTING.md](SELF_HOSTING.md).

> Scope note: this file is a depth companion to `SELF_HOSTING.md`.
...
- `SELF_HOSTING.md` defines the generic repo-supported self-host workflow
- this file expands advanced configuration for that generic self-host path
...
- This file may document generic self-host variables, reverse proxy setup,
  database choices, and LAN exposure patterns.
- This file should not redefine the current Mortis private production binds,
  domains, or operator workflow as if they were generic defaults.
- Generic examples such as `3000/8080` are compatibility examples for self-host
  users, not current Mortis private deployment truth.
...
- Use this file for deeper self-host configuration knobs.
- Use `SELF_HOSTING.md` for the supported generic self-host path.
- Use `MORTIS_PRIVATE_DEPLOYMENT_NOTES.md` for the current private deployment
  runtime facts and operator shortcuts.
